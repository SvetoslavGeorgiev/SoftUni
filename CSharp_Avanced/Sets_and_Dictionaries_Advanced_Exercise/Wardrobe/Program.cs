...
namespace Wardrobe
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            {
                var currentLine = Console.ReadLine().Split(" -> ", StringSplitOptions.RemoveEmptyEntries).ToArray();

                var collor = currentLine[0];
                var clothes = currentLine[1].Split(',', StringSplitOptions.RemoveEmptyEntries).ToArray();
            ...
            var itemLookingFor = Console.ReadLine().Split(" ", StringSplitOptions.RemoveEmptyEntries).ToArray();
